...
#### Why this happens? 
So, this is done by the UpperBody Animation Layer that we added to our animation controller, as it's mask was defined with our UpperBody Animation Mask that only consider upper body parts of the humanoid, Unity overrides the base layer animation with the upper body part of the "Idle_GunMiddle".

## Making the robot shoot
To better understand how the Avatar Mask and Animation Layer works to override the animations from the base layer with the animations from UpperBody layer we will make the robot shoot when the button "SHOOT / SINGLE" became checked.
...
## Challenge
Do something is better to learn than just read and follow someone instructions, because this I will let you try to implement the another actions:
...
To implement this actions we need to do almost same thing we did in this tutorial so far, just using another animation clips, transitions and parameters.

The challenger are the legs, because you will need to create news Avatar Masks with the "Transform" option of the avatar configuration to get the right animation. New Animation Layers will be need as well.
...
In this tutorial we learn how to use Avatar Masks and Animation Layers. These techiniques allowing us to use already existent animations and combine thems.
